...
namespace QgsWms
{
  ...
  namespace
  {
    ...
    void appendLayerStyles( QDomDocument &doc, QDomElement &layerElem, const layerInfos &linfos,
    ...
    void appendLayersFromTreeGroup( QDomDocument &doc,
                                    ...
                                    const QMap< QString, layerInfos > &wmsLayerInfos,
  ...
  void handleLayersFromTreeGroup( QDomDocument &doc,
                                  ...
                                  bool projectSettings )
  {
    auto layerIds = layerTreeGroup->findLayerIds();

    if ( hasQueryableLayers( layerIds, wmsLayerInfos ) )
    {
      parentLayer.setAttribute( QStringLiteral( "queryable" ), QStringLiteral( "1" ) );
    }
    else
    {
      parentLayer.setAttribute( QStringLiteral( "queryable" ), QStringLiteral( "0" ) );
    }

    QgsRectangle wgs84BoundingRect = combineWgs84BoundingRect( layerIds, wmsLayerInfos );
      ...
      const QgsWmsRequest &request, bool projectSettings )
  {
    ...
    // Instanciate CRS's from the project's crs list
    // This will prevent us to reinstanciate all the crs's each
    ...
    {
      auto crs = QgsCoordinateReferenceSystem::fromOgcWmsCrs( crsdef );
      if ( crs.isValid() )
      {
    ...
    // Get WMS layer infos
    QMap< QString, layerInfos > wmsLayerInfos = getWmsLayerInfos( serverIface, project, outputCrsList );
    ...
    if ( !wmsExtent.isEmpty() )
    {
      ...
      try
      {
        wmsWgs84BoundingRect = transformExtent(
      ...
      catch ( QgsCsException &cse )
      {
        ...
        wmsWgs84BoundingRect = QgsRectangle();
      }
      ...
      try
      {
        wmsCrsExtents = transformExtentToCrsList(
      ...
      if ( hasQueryableLayers( projectLayerTreeRoot->findLayerIds(), wmsLayerInfos ) )
      {
  ...
  namespace
  {
                                    ...
                                    bool projectSettings )
    {
      ...
      for ( int i = 0; i < layerTreeGroupChildren.size(); ++i )
      {
        ...
        else
        {
          ...
          const layerInfos &linfos = wmsLayerInfos[ treeLayer->layerId() ];
          ...
          QDomElement nameElem = doc.createElement( QStringLiteral( "Name" ) );
          QDomText nameText = doc.createTextNode( linfos.name );
          nameElem.appendChild( nameText );
          layerElem.appendChild( nameElem );

          QDomElement titleElem = doc.createElement( QStringLiteral( "Title" ) );
          QDomText titleText = doc.createTextNode( linfos.title );
          titleElem.appendChild( titleText );
          layerElem.appendChild( titleElem );

          if ( ! linfos.abstract.isEmpty() )
          {
            QDomElement abstractElem = doc.createElement( QStringLiteral( "Abstract" ) );
            QDomText abstractText = doc.createTextNode( linfos.abstract );
            abstractElem.appendChild( abstractText );
            layerElem.appendChild( abstractElem );
          }

          //keyword list
          if ( ! linfos.keywords.isEmpty() )
          {
            QDomElement keywordListElem = doc.createElement( QStringLiteral( "KeywordList" ) );
            for ( const QString &keyword : linfos.keywords )
            {
          ...
          if ( ! linfos.wgs84BoundingRect.isNull() )
          {
            appendCrsElementsToLayer( doc, layerElem, linfos.crsExtents.keys(), QStringList() );

            appendLayerWgs84BoundingRect( doc, layerElem, linfos.wgs84BoundingRect );

            appendLayerCrsExtents( doc, layerElem, linfos.crsExtents );
          }

          // add details about supported styles of the layer
          appendLayerStyles( doc, layerElem, linfos, project, request, serverIface->serverSettings() );

          //min/max scale denominatorScaleBasedVisibility
          if ( linfos.minScale >= 0.0 && linfos.maxScale >= 0.0 )
          {
            if ( version == QLatin1String( "1.1.1" ) )
            {
              ...
              scaleHintElem.setAttribute( QStringLiteral( "min" ), QString::number( linfos.maxScale * SCALE_TO_SCALEHINT ) );
              scaleHintElem.setAttribute( QStringLiteral( "max" ), QString::number( linfos.minScale * SCALE_TO_SCALEHINT ) );
              layerElem.appendChild( scaleHintElem );
            }
            else
            {
              QDomElement minScaleElem = doc.createElement( QStringLiteral( "MinScaleDenominator" ) );
              QDomText minScaleText = doc.createTextNode( QString::number( linfos.maxScale ) );
              ...
              QDomText maxScaleText = doc.createTextNode( QString::number( linfos.minScale ) );
          ...
          if ( !linfos.dataUrl.isEmpty() )
          {
            ...
            dataORElem.setAttribute( QStringLiteral( "xlink:href" ), linfos.dataUrl );
            dataUrlElem.appendChild( dataORElem );
            layerElem.appendChild( dataUrlElem );
          }

          // layer attribution
          if ( ! linfos.attribution.isEmpty() )
          {
            ...
            QDomText attribText = doc.createTextNode( linfos.attribution );
            attribTitleElem.appendChild( attribText );
            attribElem.appendChild( attribTitleElem );
            if ( ! linfos.attributionUrl.isEmpty() )
            {
              ...
              attribORElem.setAttribute( QStringLiteral( "xlink:href" ), linfos.attributionUrl );
          ...
          // layer metadata URL
          for ( const QgsMapLayerServerProperties::MetadataUrl &metadataUrl : linfos.metadataUrls )
          {
                            ...
                            const QgsProject *project, const QgsWmsRequest &request, const QgsServerSettings *settings )
    {
      ...
      QString hrefString = href.toString();
      hrefString.append( href.hasQuery() ? "&" : "?" );
      for ( const QString &styleName : linfos.styles )
      {
        ...
        QString customHrefString = linfos.legendUrl;

        QStringList getLayerLegendGraphicFormats;
        if ( !customHrefString.isEmpty() )
        {
          getLayerLegendGraphicFormats << linfos.legendUrlFormat;
        }
        else
        {
        ...
        for ( const QString &getLayerLegendGraphicFormat : getLayerLegendGraphicFormats )
        {
        ...
        if ( customHrefString.isEmpty() )
        {
          ...
          mapUrlQuery.addQueryItem( QStringLiteral( "LAYER" ), linfos.name );
    ...
    void appendLayerCrsExtents( QDomDocument &doc, QDomElement &layerElem, const QMap<QString, QgsRectangle> crsExtents )
    {
      ...
      for ( const QString &crsText : crsExtents.keys() )
      {
  ...
  bool hasQueryableLayers( const QStringList &layerIds, const QMap< QString, layerInfos > &wmsLayerInfos )
  {
  ...
  QgsRectangle combineWgs84BoundingRect( const QStringList &layerIds, const QMap< QString, layerInfos > &wmsLayerInfos )
  {
  ...
  QMap<QString, QgsRectangle> combineCrsExtents( const QStringList &layerIds, const QMap< QString, layerInfos > &wmsLayerInfos )
  {
    ...
    for ( const QString &id : layerIds )
    {
      ...
      const layerInfos &li = wmsLayerInfos[ id ];
      for ( const QString &crs : li.crsExtents.keys() )
      {
        QgsRectangle rect = li.crsExtents[ crs ];
        if ( rect.isNull() )
        {
          continue;
        }
